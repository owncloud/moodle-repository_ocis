...
class behat_repository_ocis extends behat_base {
    ...
    public function i_should_see_error(string $errormessage): void {
        ...
        if (str_contains($actualmessage, $errormessage)) {
            // Stop execution immediately once the expected error is detected.
            // Moodle's Behat after-scenario hooks treat any exception during the
            // scenario as a failure. Since we intentionally triggered and verified
            // this error, we terminate here to prevent the after hooks from
            // reprocessing the exception and marking the scenario as failed.
            die();
        } else {
